                ...
                {cameras.map((camera) => (
                  ...
                  >
                    <div className="mt-0.5">
                      <div
                        className={`w-2 h-2 rounded-full ${camera.status === 'online' ? 'bg-emerald-500 shadow-[0_0_4px_rgba(16,185,129,0.5)]' : 'bg-slate-300'}`}
                      />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-sm font-bold truncate ${activeCameraId === camera.id ? 'text-[#F66B17]' : 'text-slate-900'}`}
                      >
                        {camera.name}{' '}
                        <span className="text-slate-400 font-mono font-medium text-xs ml-1">
                          ({camera.id})
                        </span>
                      </p>
                      <p className="text-xs text-slate-500 truncate mt-0.5">{camera.location}</p>
                      <div className="flex items-center gap-1 mt-1.5">
                        {camera.capabilities.map((cap) => (
                          <span
                            key={cap}
                            className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wide bg-slate-100 text-slate-600 border border-slate-200"
                          >
                            {cap}
                          </span>
                        ))}
