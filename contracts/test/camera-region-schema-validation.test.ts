...
test('rejects malformed camera region configuration structure', () => {
  ...
  const invalidConfigurations: Array<[string, unknown]> = [
    ...
    [
      ...
      { ...configuration, regions: [without(firstRegion, 'geometryVersion')] },
    ],
    [
      'missing coordinateSpace',
      { ...configuration, regions: [without(firstRegion, 'coordinateSpace')] },
    ],
    ...
    [
      ...
      {
        ...configuration,
        regions: [{ ...firstRegion, polygon: without(firstRegion.polygon, 'coordinates') }],
      },
    ],
    ['configurationVersion zero', { ...configuration, configurationVersion: 0 }],
    [
      'unsafe configurationVersion',
      { ...configuration, configurationVersion: Number.MAX_SAFE_INTEGER + 1 },
    ],
    ...
    [
      'more than 64 regions',
      { ...configuration, regions: Array.from({ length: 65 }, (_, index) => region(index)) },
    ],
    [
      'duplicate region content',
      {
        ...configuration,
        regions: [
          firstRegion,
          { ...firstRegion, polygon: { coordinates: [...firstRegion.polygon.coordinates] } },
        ],
      },
    ],
    [
      'inactive-region property',
      { ...configuration, regions: [{ ...firstRegion, isActive: false }] },
    ],
    [
      'geometryVersion zero',
      { ...configuration, regions: [{ ...firstRegion, geometryVersion: 0 }] },
    ],
    [
      'unsafe geometryVersion',
      {
        ...configuration,
        regions: [{ ...firstRegion, geometryVersion: Number.MAX_SAFE_INTEGER + 1 }],
      },
    ],
    [
      'two vertices',
    ...
    [
      '65 vertices',
      {
        ...
        regions: [
          {
            ...firstRegion,
            polygon: { coordinates: Array.from({ length: 65 }, (_, index) => [index / 64, 0.1]) },
          },
        ],
      },
    ],
    ...
    [
      'unexpected region property',
      { ...configuration, regions: [{ ...firstRegion, unexpected: true }] },
    ],
    [
      'unexpected polygon property',
      {
        ...configuration,
        regions: [{ ...firstRegion, polygon: { ...firstRegion.polygon, unexpected: true } }],
      },
    ],
  ];
