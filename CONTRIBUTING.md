...
2. Tạo nhánh ngắn từ `main` theo quy ước: `feat/<feature-name>`, `fix/<bug-name>` hoặc `docs/<topic>`, ví dụ `feat/mf05-alert-list` hoặc `fix/mf06-unknown-identity`. Coding agent có thể dùng prefix bắt buộc của công cụ.
...
CI ứng dụng chạy lint, typecheck, tests, build/export và Docker smoke. Trước PR chạy `pnpm check`, `pnpm peers check` và các bước tích hợp liên quan. Không gọi API trả phí trong tests; báo rõ phần chưa kiểm thử trên thiết bị/GPU.
